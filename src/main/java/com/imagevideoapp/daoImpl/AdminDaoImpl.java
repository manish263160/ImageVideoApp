...
/*		}else {
		...
		try {
			if(catId != null && !catId.trim().equals("")) {
				query ="select ct.name as category_name ,uv.* from "+tablename+" uv left join categories ct on uv.category_id =ct.id where ct.id=? and uv.category_id >=0  group by uv.video_link order by uv.created_on desc limit 500";
				list = getJdbcTemplate().query(query,
						new BeanPropertyRowMapper<UploadedVideo>(UploadedVideo.class ) , catId.trim());
			}else {
			query ="select ct.name as category_name ,uv.* from "+tablename+" uv left join categories ct on uv.category_id =ct.id where uv.category_id >=0  group by uv.video_link order by uv.created_on desc limit 500";
			list = getJdbcTemplate().query(query,
