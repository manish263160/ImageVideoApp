...
/*		}else {
		...
		try {
			if(catId != null && !catId.trim().equals("")) {
				query ="select ct.name as category_name ,uv.* from "+tablename+" uv left join categories ct on uv.category_id =ct.id where ct.id=? group by uv.id order by uv.created_on desc limit 800";
				list = getJdbcTemplate().query(query.toString(),
						new BeanPropertyRowMapper<UploadedVideo>(UploadedVideo.class ) , catId.trim());
			}else {
			query ="select ct.name as category_name ,uv.* from "+tablename+" uv left join categories ct on uv.category_id =ct.id group by uv.id order by uv.created_on desc limit 800";
			list = getJdbcTemplate().query(query.toString(),
